...
| **Assistant** | A separate, project-less Assistant surface backed by the Partner assistant-api (baker-internal, headless Claude) with multiple local sessions, per-session transcripts, foreground streams, and detachable background runs (gated on the endpoint's `/v1/capabilities`; the Partner assistant-api does not offer them) that can be reconciled after restart. |
...
| GET | `/api/assistant/sessions/:id` | Load one session, its transcript, latest run, and the endpoint `capabilities` (`backgroundRuns`, `runStop`) |
| GET | `/api/assistant/capabilities` | What the configured endpoint can do, read from its `/v1/capabilities` and cached for a minute: `{ configured, reachable, capabilities }` |
...
| POST | `/api/assistant/sessions/:id/runs` | Start a detached background run for a session. 501 when the endpoint reports no `run_submission` (the Partner assistant-api does not), 503 when its capabilities cannot be read |
| GET | `/api/assistant/runs/:runId` | Read local run state, refreshed from the Partner when possible |
| POST | `/api/assistant/runs/:runId/stop` | Stop a remote run. 501 when the endpoint reports no `run_stop` |
| POST | `/api/assistant/sync` | Reconcile running Assistant runs with the endpoint. Against an endpoint without background runs, leftover running rows settle as `unknown` once; an unreadable endpoint leaves them alone |
