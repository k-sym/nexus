...
Cards show the origin (Jira key, `#N`, Monday item, or nothing for a plain chat), the last model, the last activity, and the linked Monday item. The **+** on the board files an idea in Idea Watcher rather than parking a card: graduated ideas arrive in the Inbox as issues on their own. The desktop icon beside it is **Import from Claude Desktop**: a picker of the Claude Desktop and terminal sessions under the project's repo path that are not on the board yet (see *Claude Desktop handoff* under Engines); a card imported or handed off that way carries a **Desktop** chip.
...
Both engines share the tool policy, Supervise, the approval and question brokers, the audit trail, memory recall and the Nexus tools (offered to Claude as an in-process MCP server). Transcripts for both live in `~/.nexus/sessions/<repo>/…jsonl`; the Claude engine additionally keeps the SDK's own session under `~/.claude/projects/` and removes it when the thread is deleted or archived, unless the session is shared with the Claude Desktop app (below), in which case it is left in place.

**Claude Desktop handoff.** A Claude-engine session and a Claude Desktop (Code tab) session are the same kind of transcript under `~/.claude/projects/<cwd-slug>/<id>.jsonl` on the machine running the backend, so the two can share one. **Open in Claude Desktop** in the chat header (web) or the session menu (iOS) asks the desktop app to adopt the session through its `claude://resume?session=<id>` link; the thread stays live in Nexus on the same id, and turns made in the desktop appear in Nexus the next time the thread is opened or prompted (Nexus re-reads the transcript past a cursor it keeps in the thread's JSONL, so nothing is replayed twice). **Import from Claude Desktop** on the board lists the desktop app's and the terminal's sessions for the project's repo path (Nexus's own are hidden, as are ones already on the board) and creates a thread that continues the chosen one with full context; the desktop can still resume it afterwards. Both stamp `chat_threads.desktop_shared_at`, which drives the **Desktop** chip and stops delete from removing the shared transcript. Two writers are not serialised, and the transcript is a tree (each entry names its parent), so the last writer's chain is what the model sees next: Nexus resumes from disk on every turn and so continues whatever the desktop wrote, but the desktop app holds an open session in memory between turns and answers on its own branch, past any turn Nexus made since the app last loaded the session — reopen the session there (or restart the app) to pick those up. Nexus's mirror is keyed on the response ids it already holds, so it shows both branches in the order they landed; a Nexus turn also waits for the transcript to go quiet before it resumes. Routes: `POST /api/threads/:id/desktop/open`, `GET /api/projects/:id/desktop/sessions`, `POST /api/projects/:id/desktop/sessions/:sessionId/import`. The Settings **Engines** card says whether the desktop app and its session index were found on this host; the open action is disabled when the app is absent, and nothing is configured in `config.yaml`.
...
- `skills` (default `'all'`) controls which of the loaded skills are available to the model: `all`, `none`, or an explicit list of skill names. `none` also disables Claude Code's bundled skills and workflows outright (the SDK's `disableBundledSkills`, equivalent to `CLAUDE_CODE_DISABLE_BUNDLED_SKILLS=1`); a list only filters which discovered skills — bundled or not — are visible to the model.
- **Context files.** Claude sessions always receive AGENTS.md/CLAUDE.md from the project directory and its ancestors (e.g. a `~/AGENTS.md`), the same lookup Pi sessions use: the first `AGENTS.override.md`/`AGENTS.md`/`CLAUDE.md` found in the project directory and in each ancestor directory up to `/`, plus one from `~/.nexus/sessions`. `CLAUDE.md` is included too, *unless* `project` is one of the `setting_sources` — in that case the SDK loads `CLAUDE.md` itself and Nexus skips it to avoid sending it twice. Pi sessions are unaffected; they already load both files directly.
...
- **Repo skills.** Nexus ships `nexus-idea-research` and `nexus-idea-graduate` in `.claude/skills/` for Idea Watcher threads; the development loop itself (`sdlc-intent`, `sdlc-spec`, `sdlc-bugfix`, `sdlc-ship`) is the shared set in `baker-internal/skills/`, driven by the Development conventions section of AGENTS.md. `./scripts/link-skills.sh` symlinks the repo's skills into `~/.claude/skills` so `setting_sources: [user]` sessions and the partner see them on baker-pro.
- The Settings **Engines** section shows the effective values in words: `Settings loaded: none (isolated)` or a comma-joined list like `Settings loaded: user, project`, and `Skills: all` / `Skills: none (bundled skills disabled)` / `Skills: 3 listed`.
...
| `monday_search` / `monday_get_item` / `monday_post_update` | Read (and, if opted in, post updates to) the Monday.com initiative a task is linked to. Posts go through the same per-item throttle as Nexus's own Review/Deploy notes, so the tool may report a note as queued rather than posted. | Monday is configured and the task has a linked item. |
...
The partner's routines post **attention items** (baker-internal#140): what needs Keith — waiting mail, a pending draft, a meeting pack, an autonomy proposal, quiz prep — with a one-line why, the proposed next action and the verbs each surface may offer. Nexus stores nothing about them except a push cursor; the partner is canonical and the client never invents a verb. The backend proxies them at `/api/attention` (see the API table) and `src/backend/attention/poll.ts` lists the `open` items every `attention.poll_minutes`, sending one APNs push per item whose `alert_seq` moved past the cursor in `attention_push_cursor` — deep link `attention:<id>`, notification thread `attention:<kind>`, badge = pending approvals + open items (the same sum the approval and run-finished pushes now carry). The first tick after a fresh database, and any tick where the partner's counter is below the cursor (a wiped partner store), seed the cursor silently rather than pushing the backlog. A partner outage logs one line per distinct message. `attention.push: false` keeps the tick (badge count, cursor) and only stops the notify. On the phone (`ios/`), `LiveHub` polls the list at app level and sums approvals + open items into the app-icon badge; the **Assistant tab** is two sections — *Sessions* (the partner conversations, named for what they are: the partner's title, else its preview, else "Partner conversation · TUI/CLI/API") and *Needs you* (one row per live item with the kind glyph, why and proposed verb; the tab badge is the open count) — and the item sheet renders exactly the item's verbs plus three client follow-ups — **Show the page** (the vault page behind a meeting item, fetched by title from the memory daemon via `GET /api/attention/:id/page` and rendered as markdown), **Ask the partner** (the partner's current conversation opened with the item as the first message) and **File as a to-do** (`POST /api/attention/:id/file { project_id }` → a Board session on that project stamped with the item as its origin, `attention_item` on `chat_threads`, seeded with a composed first turn; the item is dismissed on the partner). The **File…** sheet also offers two destinations for items that belong to no project: **Ideas** (`POST /api/attention/:id/file/idea` → a parked Idea Watcher row, source `attention`, whose notes carry the why and the item's provenance but never mail text) and **Reminder** (the phone adds it to Apple Reminders through EventKit with a due date — Tomorrow / Friday / Month end or any date — then dismisses the item). The dismiss records `filed_as`: the thread id, `idea:<id>` or `reminder`, and both clients name the outcome from it. The picker's projects come from an in-memory cache the app fills whenever it fetches projects, so it opens with rows in hand — and — `draft` shows a drafting state until the partner's item leaves `resolving`, then offers the draft for review; `open` follows the item's link; `snooze` is the partner's three presets. Verbs appear only while the item is `open` or `snoozed`; a resolved item shows its outcome. A push tap (`attention:<id>`) opens that sheet over whatever tab is showing, and a backend without the route hides the card. On the desktop, the same **Needs you** card sits first on Mission Control (`NeedsYouCard.tsx`, 60 s poll): rows expand in place with the why, the body and the item's verbs; `draft` polls until the partner's item settles and then points at the Drafts card below, where the approve verb lives. On the G2 (`src/glasses`), the same items make the **Needs-you list**: the glasses gateway (`:8899`) serves `GET /api/attention` (the partner's open items in a lens shape) and `POST /api/attention/:id/resolve` (forwarded as `by: glasses`, `surface: lens`); the list is a third home screen beside Projects and Sessions — the sessions blocking on a human, then open actions, then notices, under "NEEDS YOU · N to action · M to see" — and the cockpit opens on it when something is actionable (a notice alone does not). Tap on a row opens a session or the **item card**: the kind in the header, title and why on the left, the item's `lens_verbs` as a selectable list on the right (Draft a reply / Snooze until tomorrow / Dismiss, "Seen" on a notice); scroll picks, tap runs the verb and returns to the list, double-tap goes back — never `open`, never `close`, never a verb the glasses invent. Projects keeps a first row "★ Needs you · counts" as the door. There is no pushed hero any more (slice 7): the phone's push, mirrored to the lens by Even, is the alert. Since slice 8 the card also **reads**: a Read row first whenever there is something to read — a notice's body, a meeting item's vault page, a mail item's latest message (the last two fetched on the tap through the gateway's `GET /api/attention/:id/page` and `/thread`, the same answers the phone gets) — opening a paged text screen (seven rows a page; scroll or tap to page; on a notice's last page the tap is Seen; double-tap back). Reading records nothing. A **To-do** row files the item as a Board card from the lens (`POST /api/attention/:id/file` on the gateway, stamped `glasses`/`lens`) on the producer's suggested project or one picked from `GET /api/projects`; the lens has no chat, so the composed first turn is stored as the card's first message and the card waits on the Board for whoever opens it next. `?sim=preview` with the `needs` and `read` fixtures shows the list, the card and the read screen without a gateway.

**Inbox follow-through (slice 6c, after baker-internal's 6b).** Items carry a **category** — `action` or `notice` (a digest: morning brief, evening triage, night summary, reconciliation update, system alert) — and a notice wants one thing, **Seen** (the partner's `dismiss` under its honest name): it never pushes, never counts on the badge, ranks after actions on every surface (the phone's list, a *Notices* divider on the desktop card whose header reads "N to action · M to see", the lens list under "to see", its card's one row reading "Seen"). New action kinds: **`pr.review`** (one per PR the partner's 08:00 review says needs Keith; `links.url` is the PR) and **`recon.decision`** (one per reconciliation question or approval). The `open` verb is labelled by where it goes — **Open PR** (the browser), **Open Gap Report** / **Show the page** (the vault page), **Review the draft** — and the two writes the phone and desktop can now trigger are both confirm-gated: **Close pull request** (offered only when the item lists `close`, which the partner does for NEEDS CHANGES / RISKY verdicts only; the partner runs `gh pr close` and the item shows "Closing the PR…" until it settles; never merge, never from the lens) and **Approve cleanup** (only on the `recon.decision` keyed `recon:<statement>:cleanup`; a `dismiss` carrying `result: { approved: true }`, the exact key the reconciliation skill reads back — Nexus deletes nothing). Filing a to-do now tells the partner `result: { filed_as: <thread id> }`. The producer's `suggested_project` pre-selects the to-do picker.

**The message behind a mail item (slice 6d).** A `mail.*` item's sheet on the phone and its expanded row on the desktop show a **Latest message** section — sender, date and the body of the newest message in the conversation — fetched once when the sheet or row opens (never from the list poll) through `GET /api/attention/:id/thread`, which the partner answers by reading the thread itself. The text is for a person to read: it is never put into the Ask-the-partner seed or the to-do first turn (both name the conversation — `partner mail thread <account>:<ref>` — so an agent can read it through the partner's own guardrail). A mailbox the partner cannot read today shows its sentence under the header with the verbs intact; a backend or partner without the route hides the section. The list row itself shows the first line the partner now writes onto the item.
...
| GET | `/api/attention/:id/page` | The vault page behind the item (`links.vault_page`, resolved by exact title against the memory daemon's `global` namespace) → `{ title, body, memory_id }`. 404 when the item has no page or the vault lacks it; 502 when the daemon is down |
| GET | `/api/attention/:id/thread` | The latest message behind a `mail.*` item, read by the partner with its own `mail thread` → `{ item_id, messages: [ { account, id, thread, from, from_name, subject, date, body } ] }` (one element; plain text, HTML stripped, clipped by the partner). 404 (no such item, or a partner without the route) and 409 (not a mail item; a mailbox the partner cannot read — Google today) pass through with the partner's sentence; 502 otherwise. Read-only: nothing is recorded on the item |
| POST | `/api/attention/:id/file` | `{ project_id, by?, surface? }` → `{ thread, firstTurn }`: a Board session on the project stamped with the item as its origin, plus the composed first turn the client seeds the chat with. Dismisses the item on the partner (a refused dismiss does not undo the filing) |
| POST | `/api/attention/:id/file/idea` | `{ by?, surface? }` → `Idea`: parks the item as an Idea Watcher row (source `attention`; seed = why, non-mail body and the provenance line) and dismisses it on the partner with `{ filed_as: "idea:<id>" }` |
| POST | `/api/attention/:id/resolve` | `{ verb, by?, surface?, until?, preset?, result? }`. `verb` required (400 before any upstream call). Answers with the partner's status: 200 done, **202 a slow verb is still running** — `draft` (a reply being written) or `close` (the partner closing the PR behind a `pr.review` item with its own `gh`) — the item is `resolving`, poll the detail; 400/404/409 with the partner's sentence as `error`. `result`, a plain JSON object, rides on a `dismiss` to say what the item became (`{ approved: true }` from Approve cleanup; the file route sends `{ filed_as }`); arrays and scalars are dropped. The verb set is the partner's closed set (`draft`, `open`, `close`, `snooze`, `dismiss`); there is no approve, merge or send |
