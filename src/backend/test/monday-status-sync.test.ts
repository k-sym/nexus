...
beforeEach(() => { __resetStatusSyncState(); __resetRunRegistry(); });
...
test('deriveItemStage: no linked sessions → null', () => {
...
test('deriveItemStage: deploy only when every session is done', () => {
...
test('writeStatus writes the mapped label for the aggregate stage', async () => {
  const db = getDb(':memory:'); seedItem(db); seedSessions(db, ['in_progress']);
...
test('an item is Complete only when every linked session is done', async () => {
  const db = getDb(':memory:'); seedItem(db); seedSessions(db, ['deploy', 'in_progress']);
  const { calls, deps } = captureDeps();
  assert.equal(await writeStatus(db, OPTS, cfg(), '1', deps), 'written');
...
test('all linked sessions archived → Complete', async () => {
  const db = getDb(':memory:'); seedItem(db); seedSessions(db, ['deploy', 'deploy']);
  const { calls, deps } = captureDeps();
...
test('review outranks in-progress in the aggregate', async () => {
  const db = getDb(':memory:'); seedItem(db); seedSessions(db, ['review', 'in_progress']);
  const { calls, deps } = captureDeps();
  await writeStatus(db, OPTS, cfg(), '1', deps);
...
test('writeStatus writes only labels present in the mapping — never the inbox label', async () => {
  const db = getDb(':memory:'); seedItem(db); seedSessions(db, ['review']);
  const { calls, deps } = captureDeps();
  await writeStatus(db, OPTS, cfg(), '1', deps);
...
test('writeStatus is skipped when the item has no linked sessions', async () => {
...
test('writeStatus is a no-op when the column already shows the target', async () => {
  const db = getDb(':memory:'); seedItem(db, { statusText: 'In flight' }); seedSessions(db, ['in_progress']);
  const { calls, deps } = captureDeps();
...
test('a human-held label outside the mapping is never overwritten by a plain move', async () => {
  const db = getDb(':memory:'); seedItem(db, { statusText: 'Wants attention' }); seedSessions(db, ['in_progress']);
  const { calls, deps } = captureDeps();
  assert.equal(await writeStatus(db, OPTS, cfg(), '1', deps), 'skipped');
  assert.equal(calls.length, 0);
  db.close();
...
test('a blank status is advanceable on a plain move (not a hold)', async () => {
  const db = getDb(':memory:'); seedItem(db, { statusText: null }); seedSessions(db, ['in_progress']);
  const { calls, deps } = captureDeps();
  assert.equal(await writeStatus(db, OPTS, cfg(), '1', deps), 'written');
  assert.equal(calls[0][4], 'In flight');
  db.close();
...
test('forward-only blocks a regression to an earlier stage', async () => {
  const db = getDb(':memory:'); seedItem(db, { statusText: 'Complete' }); seedSessions(db, ['in_progress']);
  const { calls, deps } = captureDeps();
  assert.equal(await writeStatus(db, OPTS, cfg(), '1', deps), 'skipped');
  assert.equal(calls.length, 0);
  db.close();
