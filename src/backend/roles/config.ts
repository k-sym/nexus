...
export function readOverrides(raw: unknown): RoleOverrides {
  if (typeof raw !== 'string') return {};
  try { const parsed = JSON.parse(raw); return Object.fromEntries(ROLE_NAMES.filter(r => typeof parsed?.[r] === 'string').map(r => [r, parsed[r]])); } catch { return {}; }
}
...
export function validateRolesConfig(config: RolesConfig): string | undefined {
  ...
  if (!config.models || ROLE_NAMES.some(r => typeof config.models[r] !== 'string' || !config.models[r].includes('/'))) return 'Each role needs a provider/model key';
}
