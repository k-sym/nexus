...
export function labelledApprovals(broker: ApprovalBroker, role: RoleName): ApprovalBroker {
  const label = role[0].toUpperCase() + role.slice(1);
  return new Proxy(broker, { get(target, key) {
    if (key === 'register') return (...args: Parameters<ApprovalBroker['register']>) => {
      const [thread, call, tool, ...rest] = args;
      return target.register(thread, call, `${label} · ${tool}`, ...rest);
    };
