...
import { APPROVAL_DECISION_CUSTOM_TYPE, type RoleChildRun } from '@nexus/shared';
import { flattenEntries } from './chat.js';

interface RoleRunRow {
  id: string; thread_id: string; role: RoleChildRun['role']; model_key: string;
  status: RoleChildRun['status']; tokens: number | null; duration_ms: number | null;
  report: string | null; repo_path: string | null;
}
...
export async function registerRoleRunRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/api/runs/:id/events', async (request, reply) => {
    ...
    const child: RoleChildRun = { childRunId: row.id, role: row.role, model: row.model_key, status: row.status,
