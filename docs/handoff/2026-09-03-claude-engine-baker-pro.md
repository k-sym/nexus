...
1. **The live round trip never ran.** The laptop's `claude` OAuth login was expired (401) during the build. `src/backend/test/live/claude-engine.test.ts` has two cases: a plain Haiku turn plus resume, and a tool-using turn that asserts the SDK's per-content-block assistant frames were merged into one persisted assistant entry. That second assertion is what adjudicates the biggest unknown in the branch (see §5).
...
3. **Signed `npm run dist` + launch** of the packaged desktop app (checks the bundled Claude binary runs under the hardened runtime with JIT entitlements). This is a laptop/Tauri concern, not a baker-pro one — leave it to the laptop; just do not merge until it is reported done or explicitly waived.
