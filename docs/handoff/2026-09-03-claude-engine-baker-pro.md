...
2. **Manual app checks — done via the HTTP API against the baker-pro backend on 2026-09-03** (tool turn, Supervise approval + audit, `question` round trip, Stop, resume across restart, engine-mismatch 409, delete cleanup of both transcripts). Two live-only defects were found and fixed in `c662761`: the buffered assistant message must also flush on the stream's `message_stop` (a blocking MCP tool such as `question` never yields a `user` frame), and the Nexus MCP server is marked always-loaded so the model does not have to call `ToolSearch` to find `question`. Only the image turn and the visual rendering of cards remain to be eyeballed in the app.
